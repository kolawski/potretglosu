...
from settings import SAMPLES_DIRECTORY_PATH, SAMPLE_PATH_DB_KEY
...
all_psldb_paths = psldb.get_all_values_from_column(SAMPLE_PATH_DB_KEY)
all_edb_paths = edb.get_all_values_from_column(SAMPLE_PATH_DB_KEY)

print(f"psldb: {len(all_psldb_paths)}")
print(f"edb: {len(all_edb_paths)}")

difference = set(all_edb_paths) - set(all_psldb_paths)
...
# # dd = dd[~dd[SAMPLE_PATH_DB_KEY].isin(filtered_samples)]

# filtered_samples = dd[dd[F0_KEY] > 500][SAMPLE_PATH_DB_KEY].compute().to_list()

    ...
    if current_view == "input":
        ...
        return html.Div([
            ...
            html.Div(id="main-content", children=[
                ...
                html.Br(),
                html.Button("Poprzedni krok", id="previous-button", n_clicks=0, disabled=True),
...
@app.callback(
    [Output("main-content", "children"),
     ...
     Output("previous-button", "disabled")],  # Dynamiczne zarządzanie przyciskiem
...
def manage_steps(n_clicks_next, n_clicks_previous, select_clicks, selections_data, current_step):
    triggered = ctx.triggered_id
    selections = selections_data.copy()
    selected_sample_text = ""
    next_disabled = True  # Domyślnie przycisk "Dalej" jest nieaktywny
    previous_disabled = current_step == 1  # "Poprzedni krok" nieaktywny tylko w kroku 1
    ...
    if isinstance(triggered, dict) and "index" in triggered:
        ...
        return dash.no_update, dash.no_update, dash.no_update, selections, selected_sample_text, next_disabled, previous_disabled

    # Obsługa przycisków "Dalej" i "Poprzedni krok"
    if triggered == "next-button" and current_step < NUM_STEPS + 1:
        current_step += 1
    elif triggered == "previous-button" and current_step > 1:
        current_step -= 1

    previous_disabled = current_step == 1  # Aktualizacja stanu przycisku "Poprzedni krok"
    ...
    if current_step > NUM_STEPS:
        ...
        return summary_content, "Podsumowanie", current_step, selections, "", True, False
    ...
    content = html.Div([
        ...
        html.Br(),
        html.Button("Poprzedni krok", id="previous-button", n_clicks=n_clicks_previous, disabled=previous_disabled),
        html.Button(next_button_text, id="next-button", n_clicks=n_clicks_next, style={"margin-left": "10px"},
                    disabled=next_disabled)
    ])

    return content, f"Krok {current_step if current_step <= NUM_STEPS else NUM_STEPS}", current_step, selections, selected_sample_text, next_disabled, previous_disabled

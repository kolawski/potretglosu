...
def render_view(current_view, user_input, user_gender, current_step):
    if current_view == "input":
        ...
        return html.Div([
            html.H1("Etap 1. Wybór najlepszej próbki"),
            html.Div(f"Wypowiedź: {user_input}", style={"font-size": "16px", "margin-bottom": "5px"}),
            html.Div(f"Płeć: {user_gender}", style={"font-size": "16px", "margin-bottom": "20px"}),
            html.H2(id="step-header", children="Krok 1"),
            html.Div(id="main-content", children=[
                html.Div([
                    html.Div([
                        html.Div([
                            html.Div(f"Głos {idx + 1}:", style={"margin-right": "10px"}),
                            html.Audio(
                                controls=True,
                                src=f"/audio/1/{file}",
                                style={"margin-right": "10px"}
                ...
                html.Div([
                    html.H3("Wybrany głos", style={"margin-right": "10px"}),
                    html.Div(id="selected-sample", style={"font-size": "18px", "font-weight": "bold", "margin-right": "10px"}),
...
def manage_steps(n_clicks_next, select_clicks, selections_data, current_step):
    ...
    if isinstance(triggered, dict) and "index" in triggered:
        ...
        selected_sample_text = f"Głos {sample_index}"
        next_disabled = False
    ...
    content = html.Div([
        ...
        html.Div([
            html.H3("Wybrana próbka:", style={"margin-right": "10px"}),
            html.Div(
                selected_sample_text,
                id="selected-sample",
                style={"font-size": "18px", "font-weight": "bold", "margin-right": "10px"}
            ),
            html.Button(next_button_text, id="next-button", n_clicks=n_clicks_next, disabled=next_disabled)
        ], style={"display": "flex", "align-items": "center", "margin-top": "20px"})
    ])

    return content, f"Krok {current_step if current_step <= NUM_STEPS else NUM_STEPS} z 10", current_step, selections, selected_sample_text, next_disabled

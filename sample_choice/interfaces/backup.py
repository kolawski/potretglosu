...
from utils.embedding_converter import torch_to_flat
...
latent1 = torch_to_flat(latent1)
embedding1 = torch_to_flat(embedding1)
"""
...
if __name__ == "__main__":
    ...
    for step in range(10):
        ...
        for i in range(len(selected_idx)):
            ...
            euclidean_distance_embedding.append(retrieve_metrics(embedding, embedding1, get_ssim=False))
            euclidean_distance_latent.append(retrieve_metrics(latent, latent1, get_ssim=False))
        ...
        selected_distance_embedding.append(retrieve_metrics(embeddings.iloc[choice], embedding1, get_ssim=False))
        selected_distance_latent.append(retrieve_metrics(latents.iloc[choice], latent1, get_ssim=False))

        # 4. Liczymy odległość od choice do wszystkich próbek w 'indexes'
        dist_list = []
        for idx in indexes:
            d = retrieve_metrics(points[choice], points[idx])

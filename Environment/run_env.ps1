...
try {
    ...
    $runCommand = "docker run --rm -it --memory=24g --memory-swap=24g --gpus all -p 8050:8050 --entrypoint /bin/bash -v `"$hostDirectory`:$containerDirectory`" -w `"$containerDirectory`" --name $containerName $imageName"
    & cmd /c $runCommand
